...
# 创建一个日志实例
logger = Logger(logger="HandleConditionDate").getlog()
class HandleConditionDate:
    ...
    def depend_type(self,data):
        """
        判断数据的依赖类型
        """
        if self.split_data(data)[0].find("sql#")!=-1:
            return "sql"
        elif self.split_data(data)[0].find("case_") != -1:
            return "case"
        else:
            logger.error("请检查数据依赖类型的正确性")


    def depend_data(self,data,index=None):
        """
        获取到所依赖数据集合,返回case_num对应接口返回的数据
        """
        if self.split_data(data)[0].find("sql#")==-1:
            case_num=self.split_data(data)[0].strip()
            row=handle_excel.getRowsNumber(case_num,index)
            col_res=int(handle_ini.get_value("response"))
            logger.info("根据依赖case_num:{},获取到的依赖case返回数据：{}".format(case_num,handle_excel.getCellValue(row,col_res,index)))
            return handle_excel.getCellValue(row,col_res,index)
        else:
            depend_database=self.split_data(data)[0].split("#")[1].split("+")[0]
            sql_case=self.split_data(data)[0].split("#")[1].split("+")[1]
    ...
    def get_data(self,data,index=None):
        """
        传入依赖数据值，返回获取依赖数据
        """
        if self.split_data(data)[0].find("sql#")==-1:
            res_data=self.depend_data(data,index) # 返回依赖数据集合
            rule_data=self.split_data(data)[1] # 返回切割后的依赖key
    ...
    def generated_datas(self,data,sent_data=None,index=None):
        """
        ...
        for de_data in depend_case:
            if self.split_data(de_data)[0].find("sql#") == -1:  # 判断是否是case依赖
                if sent_data == None:
                    logger.error("传递参数为空，依赖值无可替换的key,请检查！")
                else:
                    if self.split_data(de_data)[2] == None:
                        logger.error("该用例为case依赖，缺少替换值参数的依赖值的key")
                    else:
                        if type(sent_data)==dict:
                            sent_data=json.dumps(sent_data)
                        if sent_data.find(self.split_data(de_data)[2]) != -1:
                            if type(self.get_data(de_data,index))==dict:
                                sent_data = sent_data.replace(self.split_data(de_data)[2],json.dumps(self.get_data(de_data,index)))
                            else:
                                sent_data = sent_data.replace(self.split_data(de_data)[2], self.get_data(de_data,index))
                        else:
                            logger.error("用例依赖替换值与参数中的替换值不一致或参数中替换值缺失，请检查！")
            else:
                old_data = self.depend_data(de_data)
                depend = eval(self.split_data(de_data)[1])
                ...
                if sent_data == None:
                    ...
                    if self.split_data(de_data)[2] == None:
                        ...
                        if sent_data.find(self.split_data(de_data)[2]) != -1:
                            sent_data = sent_data.replace(self.split_data(de_data)[2], json.dumps(depend))
                        else:
                            logger.error("sql依赖值的替换值与参数中的不一致，请检查用例！")
        return sent_data
handle_condition_data=HandleConditionDate()
